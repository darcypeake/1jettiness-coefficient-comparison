...
- `h10_qqbar.nb` — Mathematica notebook validating the ARES $h_{10}$ coefficient against the $N$-jettiness counterpart $\mathcal{C}_{-1}^{(1)}$.
- `h22-h21.nb` — Mathematica notebook computing and cross-checking the ARES $h_{21}$ and $h_{22}$ coefficients against their $N$-jettiness counterparts $\mathcal{C}_0^{(2)}$ and $\mathcal{C}_1^{(2)}$ respectively. This is done generally for any channel.
